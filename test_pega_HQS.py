...
def rouge_eval(ref_str_list, dec_str_list):
    ref_dir = "pega_ref_dir"
    dec_dir = "pega_dec_dir"
    
    for i in range(len(ref_str_list)):
        e2 = ref_str_list[i]
        with open(ref_dir + "/" + str(i) + "_reference.txt", "w") as text_file:
            text_file.write("%s" % e2)
    
    for i in range(len(dec_str_list)):
        e2 = dec_str_list[i]
        with open(dec_dir + "/" + str(i) + "_decoded.txt", "w") as text_file:
            text_file.write("%s" % e2)
...
with open('HQS_dataset/ALL_medical_term_file_train', 'r', encoding='utf8') as f:
...
args_dict = dict(
    ...
    lambda_negation=0.001
)


args_dict.update({'output_dir': 'pega_our', 'num_train_epochs':30,'train_batch_size': 4, 'eval_batch_size': 16})
...
model = model.load_from_checkpoint("pega_our/checkpointepoch_name.ckpt")
model.to('cuda')
tokenizer = PegasusTokenizer.from_pretrained('google/pegasus-large', use_fast=False)
data = get_dataset(tokenizer, "test", None, model.hparams)


dec, targets = [], []
for idx in tqdm(range(100)):
    source = data.__getitem__(idx)
    outs = model.model.generate(torch.unsqueeze(source["source_ids"].cuda(), 0), attention_mask=torch.unsqueeze(source["source_mask"].cuda(), 0), use_cache=True, decoder_attention_mask=torch.unsqueeze(source['target_mask'].cuda(), 0), max_length=84, num_beams=4, repetition_penalty=1.5, length_penalty=0.8, early_stopping=True)
    dec.append(model.ids_to_clean_text(outs)[0])

    targets_example = [tokenizer.decode(ids, skip_special_tokens=True) for ids in torch.unsqueeze(source['target_ids'], 0)]
    targets.append(targets_example[0])

rouge_eval(targets, dec)
...
with open('HQS_dataset/ALL_medical_term_file_train.txt', 'r', encoding='utf8') as f:
    custom_noun = f.readlines()
    for i in range(len(custom_noun)):
        medical_term_collection.append(custom_noun[i].replace('\n', ''))
medical_term_collection = sorted(medical_term_collection, key=lambda x: (-len(x), x))

path, total, correct, num_only_in_output = "pega_ref_dir", 0, 0, 0
for ref_filename in glob.glob(os.path.join(path, '*.txt')):
    ...
    with open(ref_filename, 'r', encoding='utf8') as f:
        see, local_term = f.readlines()[0], set()
        for term in medical_term_collection:
            if term in see:
                local_term.add(term)
                see = see.replace(term, "")
        total += len(local_term)

        output_term_set, only_in_output = set(), set()
        with open(output_file, 'r', encoding='utf8') as f2:
            ...
            output_str = whole[0]
            for term in medical_term_collection:
                if term in local_term and term in output_str:
                    output_term_set.add(term)
                    output_str = output_str.replace(term, "")
                elif term in output_str:
                    only_in_output.add(term)


        correct += len(output_term_set)
        num_only_in_output += len(only_in_output)

recall = correct/total
precision = correct/num_only_in_output
print("recall:", recall)
print("precision:", correct/num_only_in_output)
